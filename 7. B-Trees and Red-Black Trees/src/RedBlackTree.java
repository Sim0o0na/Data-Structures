...
public class RedBlackTree<T extends Comparable<T>> {
    ...
    public int getNodesCount() {
        return this.root.childrenCount;
    }
    ...
    private boolean isRed(Node node){
        ...
        return node.color == red; //if returns true == red
    }

    private void flipColors(Node node){
        node.color = red; //make node red
        node.left.color = black;//make both children black
    ...
    private Node rotateLeft(Node node){
        ...
        node.color = red;
        subtreeRoot.childrenCount = node.childrenCount;
        node.childrenCount = 1 + this.count(node.left) +  this.count(node.right);
